...
namespace hessiancsharp.client
{
    ...
    public class CHessianProxyStandardImpl : RealProxy, IRemotingTypeInfo, IHessianProxyStandard
    {
        ...
        public CHessianProxyStandardImpl(Type proxyType, CHessianProxyFactory hessianProxyFactory, Uri uri, string username, string password, 
            WebProxy webproxy, IDictionary customHeaders, bool defaultCredentials = false)
            : base(typeof(IHessianProxyStandard))
        {
            this.m_proxyType = proxyType;
            this.m_methodCaller = new CHessianMethodCaller(hessianProxyFactory, uri, username, password, webproxy, customHeaders, defaultCredentials);
            this.m_methods = proxyType.GetMethods();
        }
        ...
        public override IMessage Invoke(IMessage msg)
        {
            ...
            if (methodInfo != null)
            {
                if (methodInfo.Name.Equals("Equals") && argumentTypes != null && //$NON-NLS-1$
                ...
                else if (methodInfo.Name.Equals("GetHashCode") && argumentTypes.Length == 0) //$NON-NLS-1$
                {
                    objReturnValue = this.m_methodCaller.URI.GetHashCode();
                }
                else if (methodInfo.Name.Equals("GetHessianURL")) //$NON-NLS-1$
                {
                    objReturnValue = this.m_methodCaller.URI.ToString();
                }
                else if (methodInfo.Name.Equals("ToString") && argumentTypes.Length == 0) //$NON-NLS-1$
                {
                    objReturnValue = "[HessianProxy " + this.m_methodCaller.URI + "]"; //$NON-NLS-1$ $NON-NLS-2$
                }
                else if (methodInfo.Name.Equals("GetType") && argumentTypes.Length == 0) //$NON-NLS-1$
                {
            ...
            {
                if (methodMessage.MethodName.Equals("GetType") && (methodMessage.ArgCount == 0)) //$NON-NLS-1$
                {
                    objReturnValue = this.m_proxyType;
                }
            }
