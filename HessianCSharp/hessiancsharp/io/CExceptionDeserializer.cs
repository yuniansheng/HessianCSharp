        ...
        public override object ReadMap(AbstractHessianInput abstractHessianInput)
        {
            ...
            while (!abstractHessianInput.IsEnd())
            {
                ...
                if (objKey != null)
                {
                    ...
                    if (objKey.ToString() == "_message" || objKey.ToString() == "detailMessage") //$NON-NLS-1$ $NON-NLS-2$
                    {
                        if (field != null)
                    ...
                    else if (objKey.ToString() == "_innerException" || objKey.ToString() == "cause") //$NON-NLS-1$ $NON-NLS-2$
                    {
                        try
                        {
                            if (field != null)
            ...
            foreach (DictionaryEntry entry in fieldValueMap)
            {
                ...
                try { fieldInfo.SetValue(result, value); }
                catch (Exception) { }
            }
