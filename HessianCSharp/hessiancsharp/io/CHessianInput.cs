...
namespace hessiancsharp.io
{
    ...
    public class CHessianInput : AbstractHessianInput
    {
        ...
        public override void StartReply()
        {
            ...
            if (tag != 'r')
                throw new CHessianException("expected hessian reply"); //$NON-NLS-1$
            ...
            tag = Read();
            if (tag == 'f')
                throw new CHessianException("False Code: 'f'"); //$NON-NLS-1$
            else
                m_intPeek = tag;
        }
        ...
        private Hashtable ReadFault()
        {
            ...
            if (intCode != PROT_REPLY_END)
                throw new CHessianException("unknown fault reason"); //$NON-NLS-1$
        ...
        private int ParseChar()
        {
            while (m_intChunkLength <= 0)
            {
                ...
                switch (intCode)
                {
                    ...
                    default:
                        throw new CHessianException("unknown code:" + (char)intCode); //$NON-NLS-1$
                }
            }
            m_intChunkLength--;
        ...
        private int ParseByte()
        {
            while (m_intChunkLength <= 0)
            {
                ...
                switch (intCode)
                {
                    ...
                    default:
                        throw new CHessianException("Exception in parseByte"); //$NON-NLS-1$
                }
            }
        ...
        public override void CompleteCall()
        {
            ...
            if (intTag != 'z')
                throw new CHessianException("Expected end of call");			 //$NON-NLS-1$
        }
        ...
        public override void CompleteReply()
        {
            int tag = Read();
            if (tag != 'z')
                throw new CHessianException("Expected iend of reply");				 //$NON-NLS-1$
        }
        ...
        public override object ReadObject()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("unknown code:" + (char)intTag); //$NON-NLS-1$
            }
        }
        ...
        public override int ReadInt()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("Expected integer but found :" + intTag.ToString()); //$NON-NLS-1$
            }
        }
        ...
        public int ReadCall()
        {
            ...
            if (intTag != 'c')
            {
                throw new CHessianException("expected hessian call ('c') at code=" + intTag + " ch=" + (char)intTag);		 //$NON-NLS-1$ $NON-NLS-2$
            }
            int major = Read();
            int minor = Read();
        ...
        public String ReadMethod()
        {
            ...
            if (intTag != 'm')
            {
                throw new CHessianException("expected hessian call ('m') at code=" + intTag + " ch=" + (char)intTag);		 //$NON-NLS-1$ $NON-NLS-2$
            }
            int d1 = Read();
            int d2 = Read();
        ...
        public override string ReadString()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("expected an string but recieved " + intTag); //$NON-NLS-1$
            }
        }
        ...
        private int ParseUTF8Char()
        {
            ...
            {
                throw new CHessianException("bad utf-8 encoding"); //$NON-NLS-1$
            }
        }
        ...
        public override bool ReadBoolean()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("expected an boolean but recieved " + intTag); //$NON-NLS-1$
            }
        }
        ...
        public override double ReadDouble()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("expected an double but recieved " + intTag); //$NON-NLS-1$
            }
        }
        ...
        public override void ReadListEnd()
        {
            ...
            if (intCode != PROT_REPLY_END)
                new CHessianException("unknown code " + (char)intCode); //$NON-NLS-1$
        }
        ...
        public override byte[] ReadBytes()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("bytes " + intTag); //$NON-NLS-1$
            }
        }
        ...
        public override long ReadLong()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("expected an long but recieved " + intTag); //$NON-NLS-1$
            }
        }
        ...
        public virtual int ReadByte()
        {
            ...
            switch (intTag)
            {
                ...
                default:
                    throw new CHessianException("expected " + PROT_BINARY_END + " at " + (char)intTag); //$NON-NLS-1$ $NON-NLS-2$
            }
        }
        ...
        public virtual int ReadBytes(sbyte[] arrBuffer, int intOffset, int intLength)
        {
            ...
            else if (m_intChunkLength == 0)
            {
                ...
                switch (intTag)
                {
                    ...
                    default:
                        throw new CHessianException("expected 'B' at " + (char)intTag); //$NON-NLS-1$
                }
            }
            while (intLength > 0)
            {
                ...
                else
                {
                    ...
                    switch (tag)
                    {
                        ...
                        default:
                            throw new CHessianException("expected 'B' at " + (char)tag); //$NON-NLS-1$
                    }
                }
            }
        ...
        public override void ReadMapEnd()
        {
            ...
            if (intCode != PROT_REPLY_END)
                throw new CHessianException("expected " + PROT_REPLY_END + " at " + (char)intCode);				 //$NON-NLS-1$ $NON-NLS-2$
        }
        ...
        public override long ReadUTCDate()
        {
            ...
            if (tag != PROT_DATE_TYPE)
                throw new CHessianException("expected " + PROT_DATE_TYPE + " for Date at " + (char)tag); //$NON-NLS-1$ $NON-NLS-2$
            ...
            while (length > 0)
            {
                while (m_intChunkLength <= 0)
                {
                    ...
                    switch (code)
                    {
                        ...
                        default:
                            throw new CHessianException("expected 'byte[]' at " + (char)code); //$NON-NLS-1$
                    }
                }
        ...
        public override Stream ReadInputStream()
        {
            ...
            switch (tag)
            {
                ...
                default:
                    throw new CHessianException("expected  inputStream at " + (char)tag);								 //$NON-NLS-1$
            }
        ...
        private sealed class HessianInputStream : Stream
        {
            ...
            {
                throw new CHessianException("not-implemented"); //$NON-NLS-1$
            }
            ...
            {
                get { throw new CHessianException("not-implemented"); } //$NON-NLS-1$
            }
            ...
            {
                get { throw new CHessianException("not-implemented"); } //$NON-NLS-1$
                set { throw new CHessianException("not-implemented"); } //$NON-NLS-1$
            }
        }

        #endregion ANANYMECLASS
    }
}