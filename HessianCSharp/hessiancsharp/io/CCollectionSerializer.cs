...
namespace hessiancsharp.io
{
    ...
    public class CCollectionSerializer : AbstractSerializer
    {
        ...
        public override void WriteObject(object objList, AbstractHessianOutput abstractHessianOutput)
        {
            ...
            int collectionSize = GetCollectionSize(objList);
            Type type = objList.GetType();
            if (type.Equals(typeof(ArrayList)))
                abstractHessianOutput.WriteListBegin(collectionSize, null);
            else
                abstractHessianOutput.WriteListBegin(collectionSize, objList.GetType().FullName);
            IEnumerator enumerator = ((IEnumerable)objList).GetEnumerator();
        ...
        private int GetCollectionSize(object objList)
        {
            if (objList is ICollection)
            {
                return ((ICollection)objList).Count;
            }
            else
            {
                return (int)objList.GetType().InvokeMember("Count", System.Reflection.BindingFlags.GetProperty, null, objList, null); //$NON-NLS-1$
            }
        }
