...
namespace hessiancsharp.util
{
    ...
    public class JavaUtcConverter
    {
        ...
        public static DateTime ConvertJavaLocalTicksToLocalDateTime(long localTicks)
        {
            ...
            {
                // Ticks ausserhalb des zulässigen Bereichs
                CHessianLog.BreadcrumbEnabled = true; // falls noch nicht an, für den Retry

                throw new ArgumentOutOfRangeException("Hessian Date Deserialization Exception: Ticks out of range: C# Local Ticks = " +  //$NON-NLS-1$
                    ticks + ", Java Local Ticks " + localTicks + ", Breadcrumbs = " + CHessianLog.CurrentBreadcrumbs); //$NON-NLS-1$ $NON-NLS-2$
            }
        }
